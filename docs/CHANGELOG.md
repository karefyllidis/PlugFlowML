...
- **Neural network training script** — Removed TensorFlow/Keras from `model_training.py`. Deep models on **PyTorch** are now usable via the **`Main_6__train_evaluate_SimpleNN_IO.ipynb`** notebook (inlet→outlet baseline); `model_training.py::train_neural_network` remains a no-op CLI placeholder. No import-time TensorFlow warning.
