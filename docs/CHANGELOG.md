...
- **Docs / hygiene** — `outputs/reports/README.md` clarifies `.txt` vs `data/logs/`; `README.md`, `docs/DIRECTORY_STRUCTURE.md`, `docs/ML_CONFIG_GUIDE.md`, `docs/ml/*`, `.cursor/rules/HYDRAI_PROJECT_CONVENTIONS.mdc` aligned; dev smoke scripts listed under `scripts/dev/`.
...
- **Main_6 / Main_7 progress logging** — Removed in-notebook live §8 / §6b matplotlib dashboards. §8 appends `data/logs/<notebook_stem>_training_progress.csv`; §6b clears then incrementally updates `data/logs/<notebook_stem>_optuna_tuning_plot_data.json`. External live plots: `scripts/monitor/monitor_nn_training_progress.py` (`MAIN_6`/`MAIN_7`, optional `LIVE`; auto-picks newest log; training view shows train/test R² and gap). Utilities: `src/utils/training_progress_log.py`, `src/utils/cpu_threads.py`; dev smoke: `scripts/dev/smoke_monitor_log_compat.py`, `scripts/dev/smoke_main7_cpu_scaling.py`.
