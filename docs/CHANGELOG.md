...
### Fixed
- **Main_6/Main_7 row-cap subsampling shredded or truncated axial profiles** — the smoke/dev row cap sampled at the row level instead of the run level: Main_6 (`SUBSAMPLE_ROWS`/`runtime.subsample_max_rows`) called `DataFrame.sample()` on individual post-split `(run, z)` rows, scattering the kept rows across many runs and leaving most of them with gaps in their axial profile; Main_7 (`full_profile_max_rows`) sliced `df.iloc[:N]` in raw file order, truncating whichever run straddled row N mid-profile and yielding a non-random, order-biased subset of the design space. Both notebooks now sample whole `run_id` groups at random, budgeted by average rows/run, so a capped run is always a complete pipe profile. `docs/MODEL_CARD.md` documents the fix and flags any pre-fix artifact trained with row capping enabled for re-training.
- **NN training monitor** — `scripts/monitor/monitor_nn_training_progress.py`: correct “no logs” message uses `RUN_LABEL`; `LIVE` exits after 30s if no files (no infinite wait); robust Optuna JSON parse; CSV via `csv.reader`. Docs in `data/logs/README.md` and `docs/ML_CONFIG_GUIDE.md`.
- **Main_7 §2 CPU bootstrap** — `configure_cpu_threads` now uses `parallel_jobs=1` at startup so §3–§8 get full core budget even when `OPTUNA_N_JOBS>1` (§6b still reconfigures per trial).
- **Full-profile axial plots — inlet BC mismatch (Main_5 §8–9, Main_7 §9–9b)** — Cantera vs ML curves could diverge at the left of axial evolution figures even though run-design inputs were identical row-by-row; the surrogate was free to mis-predict state/species at the first axial station (min `relative_position`). Added `src/utils/profile_predictions.anchor_inlet_profile_predictions()` to set test predictions at each run’s inlet row to the simulated truth before metrics and overlays. Apply the same helper on cluster/forked training scripts that plot full profiles.

### Changed
- **Main_6 §8 training loop no longer early-stops** — removed the test-R² plateau/patience `break` (`EARLY_STOP_PATIENCE_EVALS`, `stall_evals`); training always runs the full configured `epochs`. Best-test-R²-checkpoint tracking and restore-before-export are unchanged. Manifest no longer records `early_stopped`.
- **Main_6 axial-station bars and exit-plane joint panels now match the NESP MLP reference look** (`main_MLP_PFR_profiles.py`) — station bars use white-fill + coloured-edge + hatch per x/L station (magenta/blue/red/black for 0.25/0.5/0.75/1.0; `AXIAL_STATION_STYLES`/`AXIAL_STATION_LABELS` centralized in `src/utils/plot_style.py`) with palegreen/yellow good/acceptable guides, replacing an earlier ad hoc sequential-Blues bar shading; exit-plane joint panels use a Cantera-blue scatter, a model-red 1:1 line, and Cantera-blue marginal histograms on both axes (previously blue-top/red-right split).
- **NN monitor logs** — Main_6 / Main_7 training progress CSV and Optuna JSON moved from `outputs/reports/` and `outputs/figures/…/` to **`data/logs/`** (`data/logs/README.md`; path helpers in `src/utils/training_progress_log.py`; monitor uses the same helpers). Removed unused **`data/raw/`**; delete legacy `optuna_tuning_plot_data.json` under `outputs/figures/<stem>/` if present.
- **Main_3 DEVEL dataset stamps** — pinned smoke-test files use `RUN_STAMP_DEVEL = 20260507_DEVEL` (`training_data_complete_*_DEVEL.pkl`); full campaign `RUN_STAMP_FULL = 20260507_095243`.
- **Docs / hygiene** — `outputs/reports/README.md` clarifies `.txt` vs `data/logs/`; monitor, DEVEL stamps, inlet anchoring, and CPU threading documented in `README.md`, `docs/DIRECTORY_STRUCTURE.md`, `docs/ML_CONFIG_GUIDE.md`, `docs/ml/*`, `data/logs/README.md`.
- **Repo hygiene** — removed stray `_main6_cells_dump/` scratch exports from the tree; `.gitignore` now ignores `*_cells_dump/` for any similar notebook cell dumps.
- **Dev scripts** — removed `scripts/dev/strip_notebook_markdown_bold.py` (one-off migration) and `scripts/dev/show_structure.sh` (redundant with `docs/STRUCTURE.md`; required external `tree`).
- **Main_6 / Main_7 progress logging** — Removed in-notebook live §8 / §6b matplotlib dashboards. §8 appends `data/logs/<notebook_stem>_training_progress.csv`; §6b clears then incrementally updates `data/logs/<notebook_stem>_optuna_tuning_plot_data.json`. External live plots: `scripts/monitor/monitor_nn_training_progress.py` (`MAIN_6`/`MAIN_7`, optional `LIVE`; auto-picks newest log; training view shows train/test R² and gap). Utilities: `src/utils/training_progress_log.py`, `src/utils/cpu_threads.py`; optional dev: `scripts/dev/smoke_main7_cpu_scaling.py`.
- **Docs** — `README.md`, `docs/ML_CONFIG_GUIDE.md`, `docs/STRUCTURE.md`, `docs/DIRECTORY_STRUCTURE.md`, `docs/ml/README.md`, and `.cursor/rules/PLUGFLOWML_NOTEBOOK_ARCHITECTURE.mdc` document monitor paths/flags, Main_7 train/val/test split and overfitting workflow; throughput flags (`USE_CUDA_AMP`, `USE_TORCH_COMPILE`, `OPTUNA_N_JOBS`, `N_CPU_CORES`).

### Added
- **Main_6 physics-aware axial-profile diagnostics** — new `src/utils/profile_evaluation.py` helper module (species-sum mass-fraction closure check, exit-plane Cantera-vs-NN joint density panels, axial NRMSE% station bars, per-station error tables), plus MC-Dropout ±σ uncertainty bands on axial overlays via `SimpleNN.predict_with_uncertainty`. New Sections 9c/9d/10d/10e gated by independent flags (`IF_SPECIES_SUM_CHECK`, `IF_MC_DROPOUT`, `IF_EXIT_JOINT_PANELS`, `IF_AXIAL_STATION_BARS`) and a new `evaluation.*` block in `configs/ml/main6_simplenn_config.json` (`axial_stations`, `nrmse_good_pct`, `nrmse_acceptable_pct`, `mc_dropout_samples`, `exit_station_tol`). Adapted from the axial-station / exit-plane diagnostics in the NESP MLP reference (`main_MLP_PFR_profiles.py`) to PlugFlowML's `relative_position` (x/L) axial coordinate. Manifest records `species_sum`, `mc_dropout`, `inlet_row_excluded`, and the `evaluation` block; axial-station metrics also export to `simple_nn_full_profile_axial_station_metrics.csv`.
- **Main_6 §4 inlet-row exclusion** — `IF_EXCLUDE_INLET_ROW` drops each run's x/L≈0 inlet row from train/test (the τ=0 state is trivially known from the design inputs); still enforced as a hard BC at inference via the existing inlet-anchoring helper (`src/utils/profile_predictions.anchor_inlet_profile_predictions`).
- **Optional Optuna hyperparameter tuning in Main_6** — new Section 6b in `notebooks/Main_6_train_evaluate_SimpleNN_IO.ipynb`, gated by `IF_HYPERPARAM_TUNING` (Section 2). When enabled, an Optuna TPE study with a median pruner searches over `h1, h2, h3, h4, dropout, learning_rate, batch_size` on a validation fold carved from the training split (test set stays held out) and maximises validation R² (uniform average, physical units). Best hyperparameters overwrite the notebook-level `H1/H2/H3/H4/DROPOUT/LEARNING_RATE/BATCH_SIZE`, the production model is rebuilt, and the rest of the notebook (tensors → training → evaluation → export) runs unchanged on the tuned net. Two new diagnostic figures (`optuna_optimization_history.png`, `optuna_param_importance.png`) and a compact `tuning` block in the export manifest (`enabled`, `n_trials_completed`, `best_val_r2`, `best_params` when tuning ran) record the search outcome. Optuna is imported only inside the guard so the baseline path keeps its existing dependency set; enable with `pip install optuna`.
- **`neural_network.tuning` config block** — `configs/ml/ml_training_config.json` exposes `n_trials` (default 30), `epochs_per_trial` (default 50), `validation_fraction` (default 0.2), and `timeout_seconds` (default `null` = no wallclock cap). Consumed by Main_6 Section 6b only when `IF_HYPERPARAM_TUNING=True`; values fall back to inline notebook defaults if a key is omitted.
- **Main_6 PyTorch NN baseline (inlet→outlet)** — `notebooks/Main_6_train_evaluate_SimpleNN_IO.ipynb` replaces the prior Iris-classification demo with a PlugFlowML multi-output regression workflow aligned with Main_4. Inputs: inlet/run-design columns only (no axial coordinates); targets: exit-plane state + species (lumped `Y_lump_*` if Main_3 produced them, else raw `Y_*`). Architecture: 4-hidden-layer ReLU MLP (defaults `128 → 64 → 32 → 16`) with `nn.Dropout(p=0.1)` between hidden blocks, `nn.MSELoss`, Adam, `TensorDataset` + `DataLoader`. Overfitting controls documented in the notebook overview: dropout, held-out test split, scalers fit on train only, shuffled minibatches, and train-vs-test convergence checkpoints. Diagnostics: 2-panel convergence plot (train + test MSE and uniform-average R² vs epoch in physical units), polished parity plots with ±5% band and hexbin density + in-axes R²/MAE, residuals-vs-actual plots with ±2σ band and bias/σ annotations, per-target R² bar chart coloured by state vs species. Architecture visualisation section gates three independent toggles: matplotlib circles+lines diagram themed by `setup_matplotlib()`, standalone TikZ `.tex` (compile externally with `pdflatex`), and optional `torchinfo` summary (`pip install torchinfo`, graceful fallback). Export emits fixed paths `models/simple_nn_exit_state_dict.pt`, `simple_nn_exit_scalers.joblib`, and `simple_nn_exit_manifest.json` (overwritten each run; records architecture incl. `h1`–`h4`, `dropout`, training config, columns, headline metrics, `run_at`, and data file).
- **`neural_network.*` config keys** — `configs/ml/ml_training_config.json` now exposes `epochs`, `batch_size`, `learning_rate`, `h1`, `h2`, `h3`, `h4`, `dropout` under `neural_network`, all consumed by Main_6 (`test_size` and `random_state` at the top level are shared with Main_4/Main_5). Notebook Section 3 "Load config & data" has a clarifying header listing exactly which keys each notebook reads, that the file is consulted only in that cell, and that missing keys fall back to inline defaults — same comment block added to Main_4 / Main_5 / Main_6 for parity.
...
### Documentation
- **Main_7 monitor & overfitting** — `docs/ML_CONFIG_GUIDE.md` adds external-monitor flags (`MAIN_6`/`MAIN_7`, `LIVE`), log paths, and Main_7 §4/§6b/§8 split; notebook opening cells point at `scripts/monitor/monitor_nn_training_progress.py`.
- **Main_6 / Main_7 / ML config docs** — `README.md`, `docs/STRUCTURE.md`, `docs/DIRECTORY_STRUCTURE.md`, `docs/ml/README.md`, and `docs/ML_CONFIG_GUIDE.md` updated for the current PyTorch notebooks: Main_6 **3-column** parity and residual grids over all targets, CSV + manifest `auxiliary_exports`; Main_7 **run-level** full-profile workflow, optional **`FULL_PROFILE_MAX_ROWS`**, **§9b** axial overlays (**state + species** along `x/L`; **fixed vs random** test-run selection), **§10** **4-column** parity with **shared hexbin colorbar** or **scatter** fallback when `n_test` is small, figure paths under `outputs/figures/Main_7_train_evaluate_SimpleNN_full_profile/`, and **`simple_nn_full_profile_*`** model + CSV exports.
...
- **Main_6 / PyTorch NN docs** — `docs/ML_CONFIG_GUIDE.md`, `docs/ml/README.md`, `docs/ml/QUICKSTART.md`, `docs/ml/IMPLEMENTATION_SUMMARY.md`, root `README.md`, and `.cursor/rules/PLUGFLOWML_PROJECT_CONVENTIONS.md` aligned with the current notebook: **three hidden layers** (`h1`–`h3`), optional Optuna over those widths only, Section 8 **ReduceLROnPlateau** on test R² checkpoints, **early stopping**, **best-checkpoint restore**, export manifest `training.{early_stopped,best_test_r2_checkpoint,best_test_r2_epoch}` and grouped state/thermo vs species `metrics`, and the per-target R² bar figure (**white bars + hatch**; neutral guide labels such as **naive baseline** at R² = 0).

### Changed
- **Main_6 PyTorch baseline (`Main_6_train_evaluate_SimpleNN_IO.ipynb`)** — `SimpleNN` is a **three-hidden-layer** MLP (defaults `128 → 64 → 32` from `neural_network.{h1,h2,h3}`; **no `h4`**). Section 8 production training uses **`ReduceLROnPlateau`** on periodic **test** R² checkpoints, **early stopping** when test R² stalls across several checkpoints, and **restores the best test-R² weights** before evaluation and export. The export manifest includes `training.{early_stopped,best_test_r2_checkpoint,best_test_r2_epoch}` and grouped `metrics.{train_r2_state,test_r2_state,train_r2_species,test_r2_species}`. Optional Optuna search (Section 6b) is over **`h1`–`h3`** only. The per-target test R² bar chart uses **white bars with hatch** by family (state/thermo vs species); vertical guide labels use neutral wording (e.g. **naive baseline** at R² = 0, **reference** / guide lines at other R²). `configs/ml/ml_training_config.json` documents **`h1`–`h3`** under `neural_network` only.
- **Stable overwrite exports** — `src.utils.run_log.start_run_log` now writes `outputs/reports/<NotebookName>.txt` in overwrite mode (one-line header; short `# end …` footer on `stop_run_log`). Re-running the setup cell closes any active tee and starts a fresh log at the same path. Main_4 / Main_5 / Main_6 model exports use fixed filenames under `models/` (`tree_models_exit.joblib`, `tree_model_tuned_exit_full.joblib`, `simple_nn_exit_*.{pt,joblib,json}`) so each full run replaces the previous artifacts; joblib payloads use `run_at` (ISO-8601) instead of a filename timestamp. Speed-report printouts in Main_4 / Main_5 are shortened to one line per task plus an optional Cantera comparison line.
- **No bold text on figures (project-wide)** - All figure elements (titles, axis labels, axis names, tick labels, legend entries, annotations, colorbar labels, suptitles) now render with `fontweight='normal'`. `setup_matplotlib()` in `src/utils/plot_style.py` locks `axes.titleweight`, `axes.labelweight`, and `figure.titleweight` to `'normal'`; `configs/style/figure_aesthetics.json` `font.title_weight` changed from `"bold"` to `"normal"`; `fontweight='bold'` stripped from `src/utils/plot_parallel.py`, `src/cantera/pfr_simulator.py` defaults, and notebooks Main_3, Main_4, Main_5. Rule documented in `.cursor/rules/PLUGFLOWML_PROJECT_CONVENTIONS.md` §5.9 (also in Critical Don'ts). Use a larger `fontsize` if more visual weight is needed.
- **Neural network training script** — Removed TensorFlow/Keras from `model_training.py`. Deep models on **PyTorch** are now usable via the **`Main_6_train_evaluate_SimpleNN_IO.ipynb`** notebook (inlet→outlet baseline); `model_training.py::train_neural_network` remains a no-op CLI placeholder. No import-time TensorFlow warning.
...
- **GitHub repository** - Public repository at https://github.com/karefyllidis/PlugFlowML
