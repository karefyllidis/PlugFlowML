...
- **Main_6 / Main_7 progress logging** — Removed in-notebook live §8 / §6b matplotlib dashboards. §8 appends `outputs/reports/<notebook_stem>_training_progress.csv`; §6b incrementally updates `optuna_tuning_plot_data.json`. External live plots: `scripts/monitor/monitor_nn_training_progress.py` (boolean flags `MAIN_6`/`MAIN_7`, `OPTUNA`, `FOLLOW`; training mode shows train/test R² and gap). Utilities: `src/utils/training_progress_log.py`, `src/utils/ml_progress_plots.py`.
- **Docs** — `README.md`, `docs/ML_CONFIG_GUIDE.md`, `docs/STRUCTURE.md`, `docs/DIRECTORY_STRUCTURE.md`, `docs/ml/README.md`, and `.cursor/rules/HYDRAI_NOTEBOOK_ARCHITECTURE.mdc` document monitor paths/flags, Main_7 train/val/test split and overfitting workflow (`OPTUNA` during §6b vs §8); throughput flags (`USE_CUDA_AMP`, `USE_TORCH_COMPILE`, `OPTUNA_N_JOBS`).
...
### Documentation
- **Main_7 monitor & overfitting** — `docs/ML_CONFIG_GUIDE.md` adds external-monitor flag table, log paths, and Main_7 §4/§6b/§8 split + when to set `OPTUNA`; notebook opening cells point at `scripts/monitor/monitor_nn_training_progress.py`.
