...
- **Docs** — `README.md`, `docs/ML_CONFIG_GUIDE.md`, `docs/STRUCTURE.md`, `docs/DIRECTORY_STRUCTURE.md`, `docs/ml/README.md`, and `.cursor/rules/HYDRAI_NOTEBOOK_ARCHITECTURE.mdc` document monitor paths/flags, Main_7 train/val/test split and overfitting workflow; throughput flags (`USE_CUDA_AMP`, `USE_TORCH_COMPILE`, `OPTUNA_N_JOBS`, `N_CPU_CORES`).

### Added
- **Main_6 physics-aware axial-profile diagnostics** — new `src/utils/profile_evaluation.py` helper module (species-sum mass-fraction closure check, exit-plane Cantera-vs-NN joint density panels, axial NRMSE% station bars, per-station error tables), plus MC-Dropout ±σ uncertainty bands on axial overlays via `SimpleNN.predict_with_uncertainty`. New Sections 9c/9d/10d/10e gated by independent flags (`IF_SPECIES_SUM_CHECK`, `IF_MC_DROPOUT`, `IF_EXIT_JOINT_PANELS`, `IF_AXIAL_STATION_BARS`) and a new `evaluation.*` block in `configs/ml/main6_simplenn_config.json` (`axial_stations`, `nrmse_good_pct`, `nrmse_acceptable_pct`, `mc_dropout_samples`, `exit_station_tol`). Adapted from the axial-station / exit-plane diagnostics in the NESP MLP reference (`main_MLP_PFR_profiles.py`) to HydrAI's `relative_position` (x/L) axial coordinate. Manifest records `species_sum`, `mc_dropout`, `inlet_row_excluded`, and the `evaluation` block; axial-station metrics also export to `simple_nn_full_profile_axial_station_metrics.csv`.
...
- **Main_6 / PyTorch NN docs** — `docs/ML_CONFIG_GUIDE.md`, `docs/ml/README.md`, `docs/ml/QUICKSTART.md`, `docs/ml/IMPLEMENTATION_SUMMARY.md`, root `README.md`, and `.cursor/rules/HYDRAI_PROJECT_CONVENTIONS.md` aligned with the current notebook: **three hidden layers** (`h1`–`h3`), optional Optuna over those widths only, Section 8 **ReduceLROnPlateau** on test R² checkpoints, **early stopping**, **best-checkpoint restore**, export manifest `training.{early_stopped,best_test_r2_checkpoint,best_test_r2_epoch}` and grouped state/thermo vs species `metrics`, and the per-target R² bar figure (**white bars + hatch**; neutral guide labels such as **naive baseline** at R² = 0).
