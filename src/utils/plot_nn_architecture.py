...
* :func:`draw_mlp_architecture` — miloharper-style schematic (empty circles
  ...
  HydrAI matplotlib aesthetics set by ``src.utils.plot_style.setup_matplotlib``.
...
and do not import ``torch``, so they can be reused by any HydrAI notebook
...
def write_tikz_mlp(
    ...
    """
    head = (
        r"% Auto-generated by HydrAI (src/utils/plot_nn_architecture.py). "
